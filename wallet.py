...
class Wallet:
	def __init__(self):
		# self.keys contains private and public key in serialized form
		# format = [[public, private], [public, private]]
		self.keys = [] # will contain key pairs as lists
		self.key_dict = {}
		
	def update_key_dict(self): # quick fix.
		self.key_dict = {key[0]: key[1] for key in self.keys}
	...
	def make_auto_transaction(self, outputs : list, amounts):
		
		self.update_key_dict() # only quick fix.
